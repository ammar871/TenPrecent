...
public class AdapterRecommend extends RecyclerView.Adapter<AdapterRecommend.ViewHolderVidio> {
...
CallFave callFave;
    ...
    int layout;
    private OnClickListenerReccommend mListenr;

    public AdapterRecommend(List<Products> list, Context mcontext, OnClickListenerReccommend mListenr, CallFave callFave) {
        this.list = list;
    ...
    public interface OnClickListenerReccommend { // create an interface
        void onClickListenerReccommend(Products position);
        void getCounterReccommend(int count,String name);// create callback function
    }

    public AdapterRecommend(List<Products> list, Context mcontext) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolderVidio holder, final int position) {
            ...
            public void onClick(View v) {
                if (shardEditor.loadDataEnter().get(ShardEditor.IS_LOGIN)){
                    ...
                    if (database.cartDao().findByName(productCart.getId())) {
                        ...
                        mListenr.getCounterReccommend(size,"تم حذف "+ " "+ list.get(position).getName());
                    } else {
                        ...
                        int size=database.cartDao().getAll().size();
                        // Toast.makeText(mcontext, ""+size, Toast.LENGTH_SHORT).show();
                        mListenr.getCounterReccommend(size, "تم اضافتها الى السلة  ");
                    }
        ...
        holder.img_fav.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (database.userDao().findByName(list.get(position).getId())){
                    ...
                    callFave.getCounterFaveRecommend(size,"تم حذفها من المفضلــة");
                }else {
                    ...
                    notifyItemChanged(position);
                    int size=database.userDao().getAll().size();
                    callFave.getCounterFaveRecommend(size,"تم اضافتهــا للمفضلة ");
                }
            }
        });
        ...
        holder.itemView.setOnClickListener(view -> mListenr.onClickListenerReccommend(list.get(position)));
    }
    @SuppressLint("UseCompatLoadingForDrawables")
