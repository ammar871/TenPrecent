...
public class AdpterProducte extends RecyclerView.Adapter<AdpterProducte.ViewHolderVidio> {
...
ShardEditor shardEditor;
    CallFave callFave;
    private OnClickListenerTheBest mListener;

    public interface OnClickListenerTheBest { // create an interface
        void onClickItemViewTheBest(Products position);
        void getCounterCartbest(int count,String name);// create callback function
    }

    public AdpterProducte(List<Products> list, Context mcontext, OnClickListenerTheBest mListener,CallFave callFave) {
        this.list = list;
...
//        Products(Integer id, String name, String desc, String image,
//                Integer categoryId, Integer sellerId, Integer orderCount, Integer rate, Integer discount)
            ...
            public void onClick(View v) {
                if (shardEditor.loadDataEnter().get(ShardEditor.IS_LOGIN)){
                    ...
                    if (database.cartDao().findByName(productCart.getId())) {
                        ...
                        mListener.getCounterCartbest(size,"تم حذف "+ " "+ list.get(position).getName());
                        ...
                        int size=database.cartDao().getAll().size();
                        // Toast.makeText(mcontext, ""+size, Toast.LENGTH_SHORT).show();
                        mListener.getCounterCartbest(size,"تم اضافتها الى السلة  ");
                    }
        ...
        holder.img_fav.setOnClickListener(v -> {
            if (shardEditor.loadDataEnter().get(ShardEditor.IS_LOGIN)){
                ...
                if (database.userDao().findByName(list.get(position).getId())) {
                    ...
                    callFave.getCounterFaveProduct(size,"تم حذفها من المفضلــة");
                    ...
                    notifyItemChanged(position);
                    int size=database.userDao().getAll().size();
                    callFave.getCounterFaveProduct(size,"تم اضافتهــا للمفضلة ");
                }
            }else {
                Toast.makeText(mcontext, "من فضلك سجل الدخول اولا", Toast.LENGTH_SHORT).show();
            }
        ...
        holder.itemView.setOnClickListener(view -> mListener.onClickItemViewTheBest(list.get(position)));
    }
...
//        else
    ...
    public static class ViewHolderVidio extends RecyclerView.ViewHolder {
        ...
        TextView tv_name, tv_price, tv_add_cart,tv_desc;
